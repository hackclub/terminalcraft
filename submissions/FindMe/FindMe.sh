...
while true; do
    ...
    loca=$(dialog --menu 'choose where to find' 0 0 3 '/' 'Find in the root dir (work on Linux and MacOs)' '/home' 'Find in the home dir for all users (Linux only)' "/home/$(whoami)" 'Find in the current user home dir (Linux only'  'Exit' "Exit the program" 3>&1 1>&2 2>&3)
    if [[ $loca == "Exit" ]]; then
clear        
break
    fi
...
if [[ "$methods" == *"Type"* ]]; then
    ...
    if (( conditions_added > 0 )); then
        ...
        if [[ -z "$results" ]]; then
            ...
            gum pager < /tmp/find_results.txt 
        fi
    else
